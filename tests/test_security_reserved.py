...
class TestSecurityReserved(unittest.TestCase):
    def test_sanitize_filename_reserved_with_empty_replacement(self):
        """
        Test that sanitize_filename handles Windows reserved names even if
        replacement is empty.
        Previously, this returned the reserved name as-is (e.g. "CON").
        """
        reserved_names = ["CON", "PRN", "AUX", "NUL", "COM1", "LPT1"]
        for name in reserved_names:
            # If replacement is empty, we expect the function to use a
            # fallback (e.g. "_") or simply prepend something to make it safe.
